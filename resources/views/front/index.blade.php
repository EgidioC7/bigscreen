    ...
    <div class="form-group">
        ...
        @if($question->question_type === 'text')
            ...
            @elseif($question->id === 20)
                <textarea class="form-control" value="{{old('question_'.$question->id)}}" name="question_{{$question->id}}" id="question_{{$question->id}}" rows="3" required></textarea>
                @if($errors->has("question_{{$question->id}}")) <span class="error">{{$errors->first("question_".$question->id)}}</span>@endif
            @else
        ...
        @elseif ($question->question_type === 'selector' )
            <select id="question_{{$question->id}}" name="question_{{$question->id}}" class="browser-default custom-select mb-4"
                required>
                <option value="0" {{ is_null(old("question_".$question->id)) ? 'selected' : '' }}>-- Choisir --</option>
                @foreach( unserialize($question->choice) as $choice)
                <option value="{{$choice}}" {{ (old("question_".$question->id) === $choice ) ? 'selected' : '' }}>{{$choice}}
                </option>
                @endforeach
            </select>
            @if($errors->has("question_{{$question->id}}")) <span class="error">{{$errors->first("question_".$question->id)}}</span>@endif
